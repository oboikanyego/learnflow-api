...
export async function getLearningIntelligence(ownerId: string) {
  ...
  const [lessons, paths, modules, goals, sessions] = await Promise.all([
    LessonModel.find({ ownerId }).select('title status durationMinutes completedAt scheduledAt learningPathId moduleId confidenceScore reviewStage nextReviewAt lastReviewedAt reviewCount').lean(),
  ...
  for (const key of completedDates) {
    ...
    longestStreak = Math.max(longestStreak, run); previous = current;
  }
  ...
  const moduleStats = modules.map(module => {
    ...
    const rated = rows.filter(l => l.confidenceScore);
    return { moduleId: String(module._id), title: module.title, total: rows.length, completed: done, missed, completionRate: rows.length ? Math.round(done / rows.length * 100) : 0, averageConfidence: rated.length ? Number((rated.reduce((s,l)=>s+(l.confidenceScore??0),0)/rated.length).toFixed(1)) : null };
  }).filter(item => item.total > 0);

  const pathStats = paths.map(path => {
    const rows = lessons.filter(l => String(l.learningPathId) === String(path._id)); const done = rows.filter(l => l.status === 'COMPLETED').length;
  ...
  return {
    ...
    weakestModules: moduleStats.filter(m => m.missed > 0 || m.completionRate < 60 || (m.averageConfidence !== null && m.averageConfidence < 3)).sort((a,b)=>(a.averageConfidence??5)-(b.averageConfidence??5)||a.completionRate-b.completionRate).slice(0,5),
...
export function coachContextFromIntelligence(value: Awaited<ReturnType<typeof getLearningIntelligence>>) {
  return JSON.stringify({ currentWeek:value.week, focus:value.focus, retention:value.retention, consistency:value.consistency, strongestPath:value.strongestPath, weakestModules:value.weakestModules, upcomingLessons:value.upcoming.slice(0,8), missedLessons:value.missedLessons, goals:value.goals }, null, 2);
}

export async function buildReplanProposal(ownerId: string) {
  const now = new Date();
  const [missed, scheduled, dueReviews, weakLessons] = await Promise.all([
    ...
    LessonModel.find({ ownerId, status:'COMPLETED', nextReviewAt:{ $lte:now } }).select('title confidenceScore nextReviewAt').sort({ confidenceScore:1,nextReviewAt:1 }).limit(10).lean(),
    LessonModel.find({ ownerId, status:'COMPLETED', confidenceScore:{ $lte:2 } }).select('title confidenceScore nextReviewAt').sort({ confidenceScore:1 }).limit(10).lean()
  ]);
  ...
  for (const lesson of missed) {
    ...
    changes.push({ lessonId:String(lesson._id), title:lesson.title, previousScheduledAt:lesson.scheduledAt, proposedScheduledAt:new Date(cursor), durationMinutes:lesson.durationMinutes }); occupied.push(cursor.getTime()); cursor = new Date(cursor.getTime()+DAY);
  }
  return { generatedAt:now.toISOString(), behindMinutes:missed.reduce((s,l)=>s+(l.durationMinutes||0),0), retentionPressure:{ dueReviews:dueReviews.length, weakLessons:weakLessons.length, recommendedReviewMinutes:Math.min(180,(dueReviews.length+weakLessons.length)*15), priorityTopics:[...new Map([...weakLessons,...dueReviews].map(l=>[String(l._id),{lessonId:String(l._id),title:l.title,confidenceScore:l.confidenceScore}])).values()].slice(0,8) }, changes };
}

export async function applyReplanProposal(ownerId: string, changes: Array<{ lessonId: string; proposedScheduledAt: string }>) {
  let updated = 0;
  for (const change of changes) { const result = await LessonModel.updateOne({ _id: change.lessonId, ownerId }, { $set: { scheduledAt:new Date(change.proposedScheduledAt), status:'SCHEDULED', reminderSentAt:null, missedAt:null } }); updated += result.modifiedCount; }
  return { updated };
}
