...
async function limits(feature: AiUsageFeature, plan: EntitlementPlan) {
  if (plan === 'PRO') return feature === 'PLAN'
    ? {
        daily: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_PRO_PLAN_DAILY),
        monthly: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_PRO_PLAN_MONTHLY)
      }
    : {
        daily: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_PRO_COACH_DAILY),
        monthly: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_PRO_COACH_MONTHLY)
      };
  return feature === 'PLAN'
    ? {
        daily: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_FREE_PLAN_DAILY),
        monthly: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_FREE_PLAN_MONTHLY)
      }
    : {
        daily: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_FREE_COACH_DAILY),
        monthly: await getSystemLimit(SYSTEM_LIMIT_KEYS.AI_FREE_COACH_MONTHLY)
      };
}
...
export async function getUserAiUsage(ownerId: string) {
  ...
  const [planLimits, coachLimits] = await Promise.all([limits('PLAN', plan), limits('COACH', plan)]);
  return {
...
export async function reserveAiUsage(ownerId: string, role: string, feature: AiUsageFeature, metadata?: Record<string, unknown>) {
  ...
  if (role !== 'admin') {
    ...
    const configured = await limits(feature, usage.entitlement.plan);
    if (featureUsage.daily.used >= configured.daily || featureUsage.monthly.used >= configured.monthly) {
      ...
      throw Object.assign(new Error(`${feature === 'PLAN' ? 'AI planner' : 'AI coach'} quota reached. Your allowance will reset automatically.`), {
        statusCode: 429,
        quota: featureUsage,
...
export async function completeAiUsage(usageId: string, status: 'SUCCEEDED' | 'FAILED', input?: { jobId?: string; errorMessage?: string }) {
  await AiUsageModel.findByIdAndUpdate(usageId, {
    status,
