...
export function redisConfigured(): boolean { return Boolean(env.REDIS_URL); }

export function createRedisConnection(options: RedisOptions = {}): IORedis {
  if (!env.REDIS_URL) throw Object.assign(new Error('Redis is not configured. Set REDIS_URL on the API service.'), { statusCode: 503 });
  return new IORedis(env.REDIS_URL, { enableReadyCheck: true, ...options });
}
...
function namespaced(key: string): string { return `${CACHE_NAMESPACE}:${key}`; }
...
export async function pingRedis(): Promise<RedisHealth> {
  ...
  try {
    ...
    return { configured: true, available: false, error: error instanceof Error ? error.message.slice(0, 160) : 'Redis unavailable' };
  }
}
...
export async function cachedJson<T>(key: string, ttlSeconds: number, loader: () => Promise<T>): Promise<T> {
  ...
  if (client) {
    ...
    } catch (error) { console.warn(`[redis-cache] read failed for ${key}: ${error instanceof Error ? error.message : String(error)}`); }
  }
  const value = await loader();
  if (client) {
    try { await client.set(redisKey, JSON.stringify(value), 'EX', ttlSeconds); }
    catch (error) { console.warn(`[redis-cache] write failed for ${key}: ${error instanceof Error ? error.message : String(error)}`); }
  }
  return value;
}
...
export async function incrementWindowCounter(key: string, windowSeconds: number): Promise<RedisWindowCounter | undefined> {
  ...
  try {
    const result = await client.eval(
      ...
      1, redisKey, String(windowSeconds)
    ) as [number, number];
...
export async function getWindowCounter(key: string): Promise<RedisWindowCounter | undefined> {
  const client = getCacheConnection();
  if (!client) return undefined;
  try {
    const redisKey = namespaced(key);
    const [rawCount, rawTtl] = await client.multi().get(redisKey).ttl(redisKey).exec() ?? [];
    const count = Number(rawCount?.[1] ?? 0);
    const ttl = Number(rawTtl?.[1] ?? 0);
    return { count: Number.isFinite(count) ? count : 0, ttlSeconds: Math.max(0, Number.isFinite(ttl) ? ttl : 0) };
  } catch (error) {
    console.warn(`[redis-cache] counter read failed for ${key}: ${error instanceof Error ? error.message : String(error)}`);
    return undefined;
  }
}
...
export async function invalidateCacheKeys(...keys: Array<string | undefined>): Promise<void> {
  ...
  try { await client.unlink(...present.map(namespaced)); }
  catch (error) { console.warn(`[redis-cache] invalidation failed: ${error instanceof Error ? error.message : String(error)}`); }
}

export async function invalidateLearningCache(ownerId: string, options: { learningPathId?: string; lessonId?: string; invalidatePathList?: boolean; invalidateAnalytics?: boolean } = {}): Promise<void> {
  const { learningPathId, lessonId, invalidatePathList = true, invalidateAnalytics = true } = options;
