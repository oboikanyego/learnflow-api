...
import { getBillingSettings, updateBillingSettings } from '../services/billing-settings.service.js';
...
export async function patchAdminBillingSettings(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    res.json(await updateBillingSettings({ ...input, updatedBy: req.user!.id }));
