...
export async function updateAdminEntitlement(req:AuthenticatedRequest,res:Response,next:NextFunction){try{const input=entitlementSchema.parse(req.body);if(input.endsAt&&input.startsAt&&input.endsAt<=input.startsAt)return res.status(400).json({message:'Entitlement end date must be after the start date.'});res.json(await changeUserEntitlement({userId:req.params.id!,changedBy:req.user!.id,...input}));}catch(error){next(error);}}

export async function getAdminEntitlementHistory(req:AuthenticatedRequest,res:Response,next:NextFunction){try{res.json(await listEntitlementHistory(req.params.id!));}catch(error){next(error);}}
