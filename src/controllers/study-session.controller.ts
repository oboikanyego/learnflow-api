...
const reflectionSchema = z.object({ reflection: z.string().trim().max(3000).optional(), confidenceScore: z.number().int().min(1).max(5).optional() });
...
async function ownedLesson(ownerId: string, lessonId: string) { return LessonModel.findOne({ _id: lessonId, ownerId }); }

export async function startStudySession(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const ownerId = req.user!.id; const lessonId = idSchema.parse(req.params.lessonId); const lesson = await ownedLesson(ownerId, lessonId);
...
export async function pauseStudySession(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const ownerId = req.user!.id; const sessionId = idSchema.parse(req.params.sessionId); const session = await StudySessionModel.findOne({ _id: sessionId, ownerId });
    if (!session) return res.status(404).json({ message: 'Study session not found' });
    if (session.status !== 'ACTIVE') return res.json(session);
    session.elapsedSeconds = elapsedNow(session); session.status = 'PAUSED'; session.pauseCount += 1; session.lastResumedAt = undefined; await session.save(); res.json(session);
  } catch (error) { next(error); }
}
...
export async function completeStudySession(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    const ownerId = req.user!.id; const sessionId = idSchema.parse(req.params.sessionId); const input = reflectionSchema.parse(req.body ?? {});
    ...
    session.status = 'COMPLETED'; session.endedAt = new Date(); session.lastResumedAt = undefined;
...
export async function abandonStudySession(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!session) return res.status(404).json({ message: 'Study session not found' });
    if (session.status === 'ACTIVE') session.elapsedSeconds = elapsedNow(session);
    session.status = 'ABANDONED'; session.endedAt = new Date(); session.lastResumedAt = undefined; await session.save(); res.json(session);
  } catch (error) { next(error); }
}

export async function getActiveStudySession(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try { const ownerId=req.user!.id; const lessonId=idSchema.parse(req.params.lessonId); const session=await StudySessionModel.findOne({ ownerId, lessonId, status:{ $in:['ACTIVE','PAUSED'] } }).sort({ createdAt:-1 }).lean(); res.json(session ? { ...session, elapsedSeconds: elapsedNow(session) } : null); }
  catch (error) { next(error); }
}
