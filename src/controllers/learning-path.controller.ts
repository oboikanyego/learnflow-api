...
export async function getLearningPath(req: AuthenticatedRequest, res: Response, next: NextFunction) { try { res.json(await service.get(req.user!.id, req.params.id)); } catch (error) { next(error); } }
...
export async function updateLearningPath(req: AuthenticatedRequest, res: Response, next: NextFunction) { try { res.json(await service.update(req.user!.id, req.params.id, updateSchema.parse(req.body))); } catch (error) { next(error); } }
export async function deleteLearningPath(req: AuthenticatedRequest, res: Response, next: NextFunction) { try { await service.remove(req.user!.id, req.params.id); res.status(204).send(); } catch (error) { next(error); } }
